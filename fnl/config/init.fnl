...
(let [options {;settings needed for compe autocompletion
...
(defn nnoremap [from to]
      (nvim.set_keymap :n from to {:noremap true :silent true}))
...
;; format code on save
(nvim.ex.augroup :formatter_autogroup ; delete whitespace after save
                 (nvim.ex.autocmd :BufWritePre "*" ":%s/\\s\\+$//e")
                 (nvim.ex.autocmd :BufWritePost "*.clj,*.fnl" :FormatWrite))
